...
const ChatRoom = props => {
  ...
  const calculateDistance = (lat1, lon1) => {
    ...
    var distanceLat = degreesToRadians(lat2-lat1);
    var distanceLon = degreesToRadians(lon2-lon1);
  ...
  //const updateUserLocation = newUserLocation => {
   // if(user = styles.isTyping){
  ...
  const getMyself = () => {
    for (var x=0;x<props.RoomUsers.length;x++) {
      if (props.RoomUsers[x].uid === props.Auth().currentUser.uid) {
        return (
          <View style={{...styles.userContainer, marginLeft: 6}} key={props.RoomUsers[x].uid}>
            <Image source={{uri: props.RoomUsers[x].photoURL}} style={styles.participantAvatar} />
            <Text style={styles.userName}>
              {props.RoomUsers[x].displayName}
            </Text>
            <Text style={{...styles.distance, color: "#434187"}}>
              {"N/A"}
            </Text>
          </View>
        )
              ...
              {props.RoomUsers.map((user, index) => (
                user.uid !== props.Auth().currentUser.uid ? (
                  <View style={styles.userContainer} key={index}>
                    <Image source={{uri: user.photoURL}} style={styles.participantAvatar} />
                    <Text style={styles.userName}>
                      {user.displayName}
                    </Text>
